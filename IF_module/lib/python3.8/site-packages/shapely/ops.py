...
class CollectionOperator:
    ...
    def linemerge(self, lines, directed=False):
        ...
        if getattr(lines, "geom_type", None) == "MultiLineString":
            ...
            source = MultiLineString([ls.z_coords for ls in lines.geoms])
        elif hasattr(lines, "__iter__"):
            try:
                source = MultiLineString([ls.z_coords for ls in lines])
    ...
    if geom.geom_type in ("Point", "LineString", "LinearRing", "Polygon"):
        ...
        try:
            if geom.geom_type in ("Point", "LineString", "LinearRing"):
                return type(geom)(zip(*func(*zip(*geom.z_coords))))
            elif geom.geom_type == "Polygon":
                shell = type(geom.exterior)(zip(*func(*zip(*geom.exterior.z_coords))))
                holes = list(
                    type(ring)(zip(*func(*zip(*ring.z_coords))))
        ...
        # TypeError, in which case we'll try again.
            ...
            if geom.geom_type in ("Point", "LineString", "LinearRing"):
                return type(geom)([func(*c) for c in geom.z_coords])
            elif geom.geom_type == "Polygon":
                shell = type(geom.exterior)([func(*c) for c in geom.exterior.z_coords])
                holes = list(
                    type(ring)([func(*c) for c in ring.z_coords])
                    for ring in geom.interiors
                )
                return type(geom)(shell, holes)
