...
def _PolygonPatch(polygon, **kwargs):
    ...
    path = Path.make_compound_path(
        Path(np.asarray(polygon.exterior.z_coords)[:, :2]),
        *[Path(np.asarray(ring.z_coords)[:, :2]) for ring in polygon.interiors],
    )
...
def _plot_linestring_collection(
    ...
    segments = [np.array(linestring.z_coords)[:, :2] for linestring in geoms]
