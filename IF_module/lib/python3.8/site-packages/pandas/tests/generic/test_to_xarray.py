...
class TestDataFrameToXArray:
    ...
    def test_to_xarray_index_types(self, index_flat, df):
        ...
        assert len(result.z_coords) == 1
        assert len(result.data_vars) == 8
        tm.assert_almost_equal(list(result.z_coords.keys()), ["foo"])
    ...
    def test_to_xarray_with_multiindex(self, df):
        ...
        assert len(result.z_coords) == 2
        assert len(result.data_vars) == 8
        tm.assert_almost_equal(list(result.z_coords.keys()), ["one", "two"])
        assert isinstance(result, Dataset)
